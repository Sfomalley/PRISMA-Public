---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Infant-Outcomes (Issued: 2024 Feb 13)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
lowbirthweight <- read.csv(paste0(path_to_data, "lowbirthweight.csv")) %>% filter(SITE != "India-SAS")
preterm_birth <- read.csv(paste0(path_to_data, "preterm_birth.csv")) %>% filter(SITE != "India-SAS")
sga <- read.csv(paste0(path_to_data, "sga.csv")) %>% filter(SITE != "India-SAS")
mortality <- read.csv(paste0(path_to_data, "mortality.csv")) %>% filter(SITE != "India-SAS")
infant_mortality  <- read.csv(paste0(path_to_data, "infant_mortality.csv")) %>% filter(SITE != "India-SAS")
neonatal_mortality <- read.csv(paste0(path_to_data, "neonatal_mortality.csv")) %>% filter(SITE != "India-SAS")
stillbirth <- read.csv(paste0(path_to_data, "stillbirth", ".csv")) %>% filter(SITE != "India-SAS")
birth_asphyxia <- read.csv(paste0(path_to_data, "birth_asphyxia", ".csv")) %>% filter(SITE != "India-SAS")
fetal_death <- read.csv(paste0(path_to_data, "fetal_death", ".csv")) %>% filter(SITE != "India-SAS")
infant_outcomes <- read.csv(paste0(path_to_data, "infant_outcomes", ".csv")) %>% filter(SITE != "India-SAS")

mnh01_constructed <- read.csv(paste0(path_to_data, "mnh01_constructed.csv")) %>% filter(SITE != "India-SAS")
mnh09_long <- read.csv(paste0(path_to_data, "mnh09_long.csv")) %>% filter(SITE != "India-SAS")
...
**Includes data from synapse last updated:** 2024 Feb 09 (2024 Feb 02 for Pakistan)
...
infantoutcomes_combined <- bind_rows(all_births, lbw_sum, preterm_sum, sga_sum, neo_mort_sum, stillbirth_sum, fetal_dth_sum, birth_asphyxia_sum) %>% select("Total", "Ghana", "India-CMC", "Kenya", "Pakistan", "Zambia")
...
tab_footnote(
 ...
 # ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is all infants with a birth outcome reported in MNH04 `(PRG_DSDECOD)` or MNH09 `(BIRTH_DSTERM)`.</span>")
 ) %>% 
  tab_footnote(
...
sga_output <- tb_theme1(sga_tab) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> R package used to generate percentiles only allows for gestational ages at birth between 33 and 42 weeks. All participants with gestational ages at birth outside this window are excluded from this table.</span>")
 )  
...
stillbirth_tab <- stillbirth %>% ## denominator is anyone with a birth outcome reported 
  ...
  summarise(
    ...
    "Fetal loss or fetal death reported but missing signs of life (NA or 77 reported)^a^ ^b^" = paste0(
      ...
      " (",
      format(round(sum(MISSING_SIGNS_OF_LIFE == 1, na.rm = TRUE)/sum(BIRTH_OUTCOME_ANY == 0, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  "Total stillbirths reported" = paste0(
   ...
   "Antepartum" = paste0(
      ...
      " (",
      format(round(sum(STILLBIRTH_TIMING == 11, na.rm = TRUE)/sum(BIRTH_OUTCOME_ANY == 0, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"), 
    
    "Intrapartum" = paste0(
      format(sum(STILLBIRTH_TIMING == 12, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(STILLBIRTH_TIMING == 12, na.rm = TRUE)/sum(BIRTH_OUTCOME_ANY == 0, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
   
    "Don't Know" = paste0(
      format(sum(STILLBIRTH_TIMING == 99, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(STILLBIRTH_TIMING == 99, na.rm = TRUE)/sum(BIRTH_OUTCOME_ANY == 0, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
**Definition:** Clinician reports failure to breathe spontaneously in the first minute after delivery or breathing assitance was required. 
