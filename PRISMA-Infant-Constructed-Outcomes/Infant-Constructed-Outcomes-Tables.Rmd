---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Infant-Outcomes (Issued: 2023-10-10)"
output:
  html_document:
    df_print: paged
    toc: yes
...
stillbirth_tab <- stillbirth %>% ## denominator is anyone with a birth outcome reported 
  ...
  summarise(
    ...
    "Fetal loss reported in MNH04 but missing MNH09^a^" = paste0(
    ...
    "GA at delivery <20 weeks^a^" = paste0(
      format(sum(GESTAGE_UNDER20 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(GESTAGE_UNDER20 == 1, na.rm = TRUE)/sum(STILLBIRTH_DENOM == 1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
stillbirth_tab %>% 
  ...
  pack_rows("Data Issues", 1, 3, label_row_css = "color: steelblue;") %>% 
  pack_rows("Stillbirth (Categorical)^a^", 4, 8, label_row_css = "color: steelblue;") %>% 
  pack_rows("Timing of Stillbirth^b^", 9,11, label_row_css = "color: steelblue;")  
