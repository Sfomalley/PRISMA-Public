...
#* Drafted: 06 October 2023, Stacie Loisate
...
## MNH04 ## 
mnh04_constructed <- enrolled_ids %>% 
  ...
  # MNH04 denominator if form is completed
  mutate(M04_FORM_COMPLETE = ifelse(!is.na(M04_MAT_VISIT_MNH04), 1, 0))  

## MNH06 ## 
mnh06_constructed<- enrolled_ids %>% 
...
## MNH08 ## 
mnh08_constructed<- enrolled_ids %>% 
  left_join(mnh08, by = c("SITE", "MOMID", "PREGID")) %>% 
  filter(M08_TYPE_VISIT==1 | is.na(M08_TYPE_VISIT)) %>% 
  # MNH08 form missing 
...
mat_infection_missingness <- enrolled_ids %>% 
  full_join(mnh04_constructed, by = c("SITE", "MOMID", "PREGID")) %>%
  filter(M04_TYPE_VISIT==1 | is.na(M04_TYPE_VISIT)) %>% 
  # join in mnh06 
...
mat_infection_diagnosed <- mnh04_constructed %>% 
  ...
  select(SITE, MOMID, PREGID, M04_TYPE_VISIT,M04_MAT_VISIT_MNH04, M04_HIV_EVER_MHOCCUR, M04_SYPH_MHOCCUR, M04_OTHR_STI_MHOCCUR, M04_GONORRHEA_MHOCCUR,
         M04_CHLAMYDIA_MHOCCUR, M04_GENULCER_MHOCCUR, M04_STI_OTHR_MHOCCUR, M04_MISSING, M04_FORM_COMPLETE) %>% 
...
mat_infection_measured<- mnh06_constructed %>%
...
## Step 3. bind and add "any measurement" variables
...
## Step 1. generate constructed vars for MNH04 diagnosed variables
mat_other_infection_mnh04  <- mnh04_constructed %>% 
  #filter(M04_TYPE_VISIT==1 | is.na(M04_TYPE_VISIT)) %>% 
...
## Step 2. generate constructed vars for MNH06 measured variables
mat_other_infection_mnh06 <- mnh06_constructed %>% 
  #filter(M06_TYPE_VISIT==1 | is.na(M06_TYPE_VISIT)) %>% 
  select(SITE, MOMID, PREGID, 
...
mat_other_infection_mnh08 <- mnh08_constructed %>% 
  #filter(M06_TYPE_VISIT==1 | is.na(M06_TYPE_VISIT)) %>% 
  select(SITE, MOMID, PREGID, 
